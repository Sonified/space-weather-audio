...
; =====================================================================
; BEGIN NEW CODE — Data interval gap detection
; =====================================================================
;
; Problem: When a requested time range has missing CDF records (e.g.
; PSP cruise phase, MMS burst modes), the WAV is built from only the
; records that exist. The resulting audio timeline silently skips over
; gaps — a 12-hour request might produce 3 hours of audio with no
; indication that 9 hours are missing. Clients have no way to know
; where data blocks start/end or how to insert silence for gaps.
;
; Solution: Detect contiguous data blocks in the epoch array by
; finding time steps that exceed 2x the median cadence. Print one
; DATAINTERVAL= line per block to STDOUT (same pattern as AUDIO=,
; STATUS=, ERROR=, etc. that the Java layer already parses).
;
; Output format (one line per contiguous block):
;   DATAINTERVAL=start_epoch,end_epoch,sample_count,median_cadence
;
; Where epochs are in native units (TT2000 nanoseconds or CDF_EPOCH
; milliseconds), and cadence is in the same units. Continuous data
; with no gaps emits exactly one DATAINTERVAL= line.
;
; Example — PSP cruise, 2 data blocks with an 8.8-hour gap:
...
IF  buf_sz gt 1 THEN BEGIN
    ...
    ; Time diffs between consecutive records (ns for TT2000, ms for CDF_EPOCH)
    time_diffs = mytime [1:*] - mytime [0:-2]

    ; Median cadence — robust against outliers
    med_cadence = MEDIAN (time_diffs)

    ; Gap threshold: any step > 2x median cadence is a gap
    ...
    FOR blk = 0, n_intervals - 1 DO BEGIN
        ...
        ; Per-block cadence (median of diffs within this block)
...
ENDIF ELSE BEGIN
    ...
    ; Single record — one interval, no cadence derivable
    n_intervals = 1
...
; Emit one DATAINTERVAL= line per block (Java layer parses these from STDOUT)
FOR blk = 0, n_intervals - 1 DO BEGIN
    PRINT, 'DATAINTERVAL=', $
...
; =====================================================================
; END NEW CODE — Data interval gap detection
; =====================================================================
